 ...
 * @param {boolean} [includeLogs=true] Whether to read all log file contents.
 * @return {Promise<Object>} Proceger task information.
 */
async function getProcegerInfo(ctx, includeLogs = true) {
    ...
    try {
      ...
      for (const pattern of patterns) {
        ...
        for (const file of files) {
          ...
          if (!foundFiles.has(base)) {
            foundFiles.set(base, file);
          }
        }
      }
      for (const [base, file] of foundFiles.entries()) {
        if (includeLogs) {
...
/**
 * Retrieves specific log content for Proceger.
 * @param {Object} ctx Apper request context.
 * @param {string} filename Log filename.
 * @param {Object} [options={}] Options such as tail count.
 * @return {Promise<Object|null>} Log content object or null.
 */
async function getProcegerLogContent(ctx, filename, options = {}) {
  ...
  for (const dir of candidateDirs) {
    ...
    if (existsSync(fullPath)) {
      try {
        ...
        if (ctx && ctx.logger) {
          ctx.logger.debug(
              `Error reading proceger log file ${fullPath}: ${err.message}`,
          );
        }
      }
    }
...
function createApp(taskManager = new TaskManager(), configOverrides = {}) {
  ...
  app.get('/task/list', async (ctx, req, res) => {
    ...
    const tasks = [];
    tasks.push(await getProcegerInfo(ctx, includeLogs));
    for (const task of taskManager.getAllTasks()) {
      tasks.push(await task.toJson({includeLogs}));
    }
    res.send(tasks);
  });

  const handleGetTaskLog = async (ctx, req, res) => {
    await ensureInit(ctx.config, ctx.logger);
    const taskName = req.params.name;
    const filename = req.params.filename;
    const tail = req.query.tail ? parseInt(req.query.tail, 10) : undefined;
    try {
      if (taskName === 'proceger') {
        const logData = await getProcegerLogContent(ctx, filename, {tail});
        if (!logData) {
      ...
      const logData = await task.getLogContent(filename, {tail});
      if (!logData) {
        return res.status(404).send({error: 'Log file not found'});
      }
      res.send(logData);
  ...
  app.get('/task/:name/log/:filename', handleGetTaskLog);
  app.get('/task/:name/logs/:filename', handleGetTaskLog);

  app.get('/task/:name', async (ctx, req, res) => {
    await ensureInit(ctx.config, ctx.logger);
    const includeLogs = req.query.includeLogs === 'true';
    if (req.params.name === 'proceger') {
      return res.send(await getProcegerInfo(ctx, includeLogs));
    }
    const task = taskManager.getTask(req.params.name);
    if (!task) {
      return res.status(404).send({error: 'Task not found'});
    }
    res.send(await task.toJson({includeLogs}));
  });
